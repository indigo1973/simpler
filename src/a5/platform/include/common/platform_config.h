...
// --- TEMPORARY L2SW-REPRO: #983-regression repro toggle, driven by the
// PTO_L2SW_REPRO env var (host reads it in device_runner). Remove after the
// hardware-cause verification is done.
//   HEAD_READ:     AICore reads the current rotating-buffer ptr cross-core from
//                  the shared L2SwimlaneActiveHead line (the #983 behavior),
//                  instead of from the per-task dispatch payload.
//   NO_HEAD_WRITE: AICPU suppresses the per-dispatch head-line write
//                  (total_record_count++), so the head line is no longer
//                  written every dispatch — isolates "concurrent write to the
//                  same line" from "reading GM" in the controlled A/B.
#define PROFILING_FLAG_L2SW_REPRO_HEAD_READ (1u << 8)
#define PROFILING_FLAG_L2SW_REPRO_NO_HEAD_WRITE (1u << 9)
