...
#include "aicore/aicore_profiling_state.h"  // L2SW-PROBE: get_aicore_profiling_flag (remove with the probe)
#include "common/platform_config.h"         // L2SW-PROBE: GET_PROFILING_FLAG + probe bit
...
__aicore__ __attribute__((always_inline)) static inline void l2_swimlane_aicore_record_task(
    ...
    // L2SW-PROBE: skip this per-task coherent re-fetch when the probe flag is
    // set, to isolate "dcci re-fetch" vs "the load" as the a5 stall trigger.
    if (!GET_PROFILING_FLAG(get_aicore_profiling_flag(), PROFILING_FLAG_L2SW_PROBE_NO_DCCI)) {
