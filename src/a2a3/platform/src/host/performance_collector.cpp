...
void ProfMemoryManager::start(
    ...
    void *user_data, int device_id, PerfSetDeviceCallback set_device_cb
) {
...
void ProfMemoryManager::free_buffer(void *dev_ptr) {
    ...
    if (unregister_cb_ != nullptr) {
        unregister_cb_(dev_ptr, device_id_, user_data_);
    }
...
int PerformanceCollector::initialize(
    ...
    PerfUnregisterCallback unregister_cb, PerfFreeCallback free_cb, void *user_data, PerfSetDeviceCallback set_device_cb
) {
    if (perf_shared_mem_host_ != nullptr) {
    ...
    // AICPU overwrites from PTO2_PERF_LEVEL after init; avoid Host defaulting to full JSON.
...
void PerformanceCollector::start_memory_manager() {
    ...
    memory_manager_.start(
        perf_shared_mem_host_, num_aicore_, PLATFORM_MAX_AICPU_THREADS, alloc_cb_, register_cb_, unregister_cb_,
        free_cb_, user_data_, device_id_, set_device_cb_
    );
}
...
int PerformanceCollector::finalize(PerfUnregisterCallback unregister_cb, PerfFreeCallback free_cb, void *user_data) {
    ...
    for (int i = 0; i < num_aicore_; i++) {
        ...
        if (state->current_buf_ptr != 0) {
            free_buf(reinterpret_cast<void *>(state->current_buf_ptr));
        }
        ...
        while (head != tail && max_iters-- > 0) {
            ...
            if (buf_ptr != 0) {
                free_buf(reinterpret_cast<void *>(buf_ptr));
            }
            head++;
        }
        if (head != tail) {
