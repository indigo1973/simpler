 ...
 *
 * AICore writes PerfRecord.mixed_task_id as the register token (low 32 bits).
 * AICPU overwrites with canonical 64-bit PTO2 raw after handshake match.
 *
 * @param perf_buf Performance buffer pointer
 * @param task_id Register dispatch id (DATA_MAIN_BASE), stored in mixed_task_id low 32 bits
...
static inline void perf_aicore_record_task(
    ...
    record->mixed_task_id = static_cast<uint64_t>(task_id);
