...
struct PerfRecord {
    ...
    // Handshake: AICore writes register token zero-extended (low 32 bits only). AICPU overwrites with
    // canonical PTO2 raw = (ring_id << 32) | local_id after FIN/perf row match (tensormap / build_graph).
    uint64_t mixed_task_id;
...
struct AicpuPhaseRecord {
    ...
    union {
        uint64_t mixed_task_id;   // Orchestrator phases: (ring_id << 32) | local_id for cross-view correlation
