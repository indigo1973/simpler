...
#if PTO2_PERF_TASK
    // --- Task-level profiling fields (dispatch path, compile-time gated) ---
...
#if PTO2_SCHED_PROFILING || PTO2_PERF_TASK || PTO2_PERF_PHASE
    bool profiling_enabled = runtime->enable_profiling;
#endif

    // Loop counter shared by scheduler summary log and phase sequence numbering.
#if PTO2_PROFILING || PTO2_PERF_PHASE
    uint64_t sched_loop_count = 0;
#endif
    // Scheduler cycle-stat counters (consumed by scheduler summary log, PTO2_PROFILING only)
#if PTO2_PROFILING
...
#    endif
#endif
    // Phase swimlane counters (consumed by perf_aicpu_record_phase, PTO2_PERF_PHASE only)
#if PTO2_PERF_PHASE
...
#    endif
        ...
        sched_loop_count++;
        uint64_t _t0_phase = profiling_enabled ? get_sys_cnt_aicpu() : 0;
#endif
...
#endif
#if PTO2_PERF_PHASE
        if (try_completed && profiling_enabled && phase_complete_count > 0) {
#    if PTO2_PROFILING
            ...
            _t0_phase = _t1;
#    else
            uint64_t _t1_c = get_sys_cnt_aicpu();
            perf_aicpu_record_phase(
                thread_idx, AicpuPhaseId::SCHED_COMPLETE, _t0_phase, _t1_c, sched_loop_count, phase_complete_count
            );
            _t0_phase = _t1_c;
#    endif
...
#endif
#if PTO2_PERF_PHASE
        if (try_pushed && profiling_enabled && phase_dispatch_count > 0) {
#    if PTO2_PROFILING
            perf_aicpu_record_phase(
            ...
            _t0_phase = _t1;
#    else
            uint64_t _t1_d = get_sys_cnt_aicpu();
            perf_aicpu_record_phase(
                thread_idx, AicpuPhaseId::SCHED_DISPATCH, _t0_phase, _t1_d, sched_loop_count, phase_dispatch_count
            );
            _t0_phase = _t1_d;
#    endif
...
#elif PTO2_PERF_PHASE
            if (profiling_enabled) {
                uint64_t _t1_i = get_sys_cnt_aicpu();
                perf_aicpu_record_phase(
                    thread_idx, AicpuPhaseId::SCHED_IDLE_WAIT, _t0_phase, _t1_i, sched_loop_count, 0
                );
                _t0_phase = _t1_i;
            }
#endif
