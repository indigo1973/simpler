...
//   1. PTO2_ORCH_PROFILING  — full ORCH sub-step cycle statistics + orch phase swimlane
//   2. PTO2_PROFILING       — no cycle stats, orch phase swimlane only (enable_profiling gated)
//   3. PTO2_PERF_LEVEL >= 2 — PTO2_PROFILING=0 but swimlane orch phase still wanted
//   4. else                 — all no-ops
//
// In branch 1: perf_aicpu_record_orch_phase is gated on (PTO2_PERF_PHASE && enable_profiling)
// so that setting PTO2_PERF_LEVEL<2 removes phase writes without disturbing cycle accumulators.
...
// Accumulate cycle stats; only write the orch phase swimlane record when
// PTO2_PERF_PHASE is compiled in AND the runtime switch is enabled.
...
#elif PTO2_PERF_LEVEL >= 2
// PTO2_PROFILING=0 but swimlane orch phase data is still wanted.
// No cycle-accumulation stats (no ORCH_PROFILING), just swimlane writes.
#    include "aicpu/device_time.h"
#    include "aicpu/performance_collector_aicpu.h"
__attribute__((weak, visibility("hidden"))) uint64_t get_sys_cnt_aicpu() { return 0; }
__attribute__((weak, visibility("hidden"))) void
...
#    endif
    g_orch_submit_idx++;
#elif PTO2_PERF_LEVEL >= 2
    // g_orch_submit_idx is declared in the PTO2_PERF_LEVEL>=2 branch above;
    // increment it here so perf_aicpu_record_orch_phase gets a unique index per task.
    g_orch_submit_idx++;
#endif
    return result;
}
