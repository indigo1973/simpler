#!/usr/bin/env bash
# Paged-attention unroll (profiling-first variant): same as run_pa_unroll.sh but Step 4 runs
# with --enable-profiling first (default 1 round), then without profiling (default 10).
# See run_pa_unroll.sh for the default order (without profiling first).
#
# Optional env:
...
#   ROUNDS_PROFILING  Number of timed rounds with --enable-profiling (default: 1)
...
ROUNDS="${ROUNDS:-10}"
ROUNDS_PROFILING="${ROUNDS_PROFILING:-1}"
...
echo "  order: with --enable-profiling ${ROUNDS_PROFILING} round(s) first, then without profiling ${ROUNDS} rounds"
echo "Device log dir: $DEVICE_LOG_DIR"

if run_timed_batch "With --enable-profiling (${ROUNDS_PROFILING} round(s))" \
...
if [[ -n "$AVG_WITH_PROF" ]]; then
    ...
    echo "  With --enable-profiling:     elapsed ${AVG_WITH_PROF} us${suf}  (${ROUNDS_PROFILING} round(s))"
else
