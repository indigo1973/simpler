#!/usr/bin/env bash
# Profiling level verification for host_build_graph runtime (paged_attention).
#
# Runs paged_attention Case1 with profiling levels 0/1/2/3 interleaved,
# then prints a summary table comparing Elapsed/Sched/Orch latencies.
#
# Usage:
#   ./run_5_hbg.sh [-d <device>] [-n <runs>] [-w <warmup>]

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
RUN_EXAMPLE="$SCRIPT_DIR/examples/scripts/run_example.py"
KERNELS_DIR="$SCRIPT_DIR/tests/st/a2a3/host_build_graph/paged_attention/kernels"
GOLDEN_PY="$SCRIPT_DIR/tests/st/a2a3/host_build_graph/paged_attention/golden.py"
...
echo ""
echo "================================================================"
echo "  Profiling Level Verification (host_build_graph)"
echo "  Test:    paged_attention / $CASE"
