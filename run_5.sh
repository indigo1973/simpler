#!/usr/bin/env bash
# Profiling overhead benchmark: measure the cost of each --enable-profiling level.
#
...
#
# Usage:
#   ./run_5.sh [-d <device>] [-n <runs>] [-w <warmup>]

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
TEST_SCRIPT=$(ls "$SCRIPT_DIR"/tests/st/a2a3/tensormap_and_ringbuffer/paged_attention_unroll/test_*.py)
...
#   Extract elapsed/sched/orch from a single-round device log.
#   Prints: elapsed_us sched_us orch_us
# ---------------------------------------------------------------------------
parse_single_round() {
    local log_file="$1"
    local timing
...
# ---------------------------------------------------------------------------
# run_one <profiling_level> [label]
#   Run test once and print the device log path. Returns timing via stdout.
# ---------------------------------------------------------------------------
run_one() {
    local level="$1"
    local label="${2:-}"
...
# Storage: RESULTS[level,run] = "elapsed sched orch"
declare -A RESULTS
...
# Compute level-0 averages for delta
declare -A AVGS
for level in "${LEVELS[@]}"; do
    se=0 ss=0 so=0
    for ((run = 1; run <= RUNS; run++)); do
