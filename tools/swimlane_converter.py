...
def format_task_display(mixed_task_id):
    """Format PTO2 mixed_task_id for human-readable labels.
    ...
    For invalid or non-numeric values, returns str(mixed_task_id).
    """
    try:
        tid = int(mixed_task_id)
    except (TypeError, ValueError):
        return str(mixed_task_id)
    if tid < 0:
        tid &= (1 << 64) - 1
    ring = (tid >> 32) & 0xFF
    ...
    if orchestrator_phases:
        ...
        for orch_idx, thread_records in enumerate(orch_threads):
            ...
            for record in thread_records:
                ...
                # Show packed mixed_task_id as rXtY / tY for task-specific phases
                if task_id >= 0:
